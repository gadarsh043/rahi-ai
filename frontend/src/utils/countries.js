/**
 * Country list for passport selector.
 * COMMON_COUNTRIES: top 10 with flag emojis.
 * ALL_COUNTRIES: full list alphabetical with flags from ISO alpha-2.
 */
...
const ISO_COUNTRIES = [
  ...
  { name: 'Congo, Democratic Republic of the', code: 'CD' }, { name: 'Costa Rica', code: 'CR' },
  ...
  { name: 'Guyana', code: 'GY' }, { name: 'Haiti', code: 'HT' }, { name: 'Holy See', code: 'VA' },
  ...
  { name: 'Kenya', code: 'KE' }, { name: 'Kiribati', code: 'KI' }, { name: "Korea, Democratic People's Republic of", code: 'KP' },
  ...
  { name: 'Mauritius', code: 'MU' }, { name: 'Mexico', code: 'MX' }, { name: 'Micronesia, Federated States of', code: 'FM' },
  { name: 'Moldova, Republic of', code: 'MD' }, { name: 'Monaco', code: 'MC' }, { name: 'Mongolia', code: 'MN' },
  ...
  { name: 'Pakistan', code: 'PK' }, { name: 'Palau', code: 'PW' }, { name: 'Palestine, State of', code: 'PS' },
  ...
  { name: 'Trinidad and Tobago', code: 'TT' }, { name: 'Tunisia', code: 'TN' }, { name: 'Türkiye', code: 'TR' },
  ...
  { name: 'Vanuatu', code: 'VU' }, { name: 'Venezuela, Bolivarian Republic of', code: 'VE' }, { name: 'Viet Nam', code: 'VN' },
...
export function filterCountries(query) {
  ...
  if (!q) return { common: COMMON_COUNTRIES, all: [] };
  const common = COMMON_COUNTRIES.filter((c) => c.name.toLowerCase().includes(q));
  const all = ALL_COUNTRIES.filter((c) => c.name.toLowerCase().includes(q) && !commonCodes.has(c.code));
  return { common, all };
}
